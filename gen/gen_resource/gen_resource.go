...
var cli_code = `
// Code generated by github.com/lujingwei002/gen_server. DO NOT EDIT.
// Code generated by github.com/lujingwei002/gen_server. DO NOT EDIT.
// Code generated by github.com/lujingwei002/gen_server. DO NOT EDIT.
...
import (
	...
	"os"
	"fmt"
	"github.com/lujingwei002/gen_server/db"
	"github.com/lujingwei002/gen_server/resource_options"
...
import (
	...
	"github.com/lujingwei002/gen_server/log"
	"os"
	"fmt"
	"context"
<<range .ImportArr>>
...
// dir - bundle所在的目录
func Push(ctx context.Context, client gen_server.DbClient, dir string, opts ...resource_options.PushOption) error {
...
type <<.LoaderStructName>> struct {
	handler gen_server.ResourceHandler
...
func New<<.LoaderStructName>>(handler gen_server.ResourceHandler) *<<.LoaderStructName>>{
...
// 从db中加载资源
func (self *<<.LoaderStructName>>) LoadFromDb(ctx context.Context, client gen_server.DbClient) error {
	<<- range .BundleArr>>
...
func (self *<<.LoaderStructName>>) LoadResource(ctx context.Context, client gen_server.DbClient, dir string, compress bool) error {
...
// 根据bundle的类型，从相应的源中加载资源
func (self *<<.LoaderStructName>>) ReloadResource(ctx context.Context, client gen_server.DbClient, dir string, compress bool) error {
	if err := self.LoadVersion(dir); err != nil {
		return err
	}
	<<- range .BundleArr>>
...
// 加载成功后，对配置进行处理
func (self *<<.LoaderStructName>>) convert(handler gen_server.ResourceHandler) error {
...
// 从db中加载资源
func (self* <<.BundleStructName>>) LoadFromDb(ctx context.Context, client gen_server.DbClient) error {
	self.Clear()
	<<- range .ResourceArr>>
...
func (self *<<.BundleStructName>>) SaveToDb(ctx context.Context, client gen_server.DbClient, dir string, opts ...resource_options.PushOption) error {
	switch c := client.(type) {
	case gen_server.MongoClient:
...
func (self *<<.ArrTypeName>>) LoadFromDb(ctx context.Context, client gen_server.DbClient) error {
	switch c := client.(type) {
	case gen_server.MongoClient:
		return self.LoadFromMongo(ctx, c.GetMongoDatabase())
	default:
		panic(gen_server.ErrDbNotSupport)
	}
}
...
func (self *<<.WrapStructName>>) LoadFromDb(ctx context.Context, client gen_server.DbClient) error {
	switch c := client.(type) {
	case gen_server.MongoClient:
		return self.LoadFromMongo(ctx, c.GetMongoDatabase())
	default:
...
func (self *<<.WrapStructName>>) Make(arr <<.ArrTypeName>>) error {
	if err := gen_server.Make<<len .MapKeyArr>>Key_<<join .MapKeyGoTypeNameArr "_">>(arr, *self<<- range $k, $v := .MapKeyArr>>, "<<camelString $v>>"<<- end>>); err != nil {
		return err
	}
	return nil
}
...
func (self *<<.WrapStructName>>) Make(arr <<.ArrTypeName>>) error {
	...
	if err := gen_server.Make<<len .ObjectKeyArr>>Key_<<join .ObjectKeyGoTypeNameArr "_">>(arr, dict<<- range $k, $v := .ObjectKeyArr>>, "<<camelString $v>>"<<- end>>); err != nil {
		return err
	}
	var ok bool
	<<- range .ValueArr>>
...
type gen_state struct {
	Config       gen_server.GenResourceConfig
	Module       string
...
// 生成协议
func Gen(config gen_server.GenResourceConfig) error {
	...
	// 	return gen_server.ErrGenNotChange
	// }
	//for _, v := range state.ResourceArr {
	// 解析excel
	//filePath := path.Join(proj.Config.ExcelDir, v.FilePath)
	...
	if err := p.parse(state); err != nil && err == gen_server.ErrGenNotChange {
