...
func Use(ctx context.Context, client gen_server.DbClient) (<<.DaoInterfaceName>>, error) {
...
func UseMongo(ctx context.Context, client gen_server.MongoClient) (*<<.MongoDaoStructName>>, error) {
	dao := NewMongoDao()
...
func UseRedis(ctx context.Context, client gen_server.RedisClient) (*<<.RedisDaoStructName>>, error) {
	dao := NewRedisDao()
	if err := dao.UseClient(client); err != nil {
		return nil, err
	}
...
func Migrate(ctx context.Context, client  gen_server.DbClient, opts ...db.MigrateOption) error {
...
func (self *<<.ArrStructName>>) Delete(<<.CapCamelSecondaryKey>> <<.SecondaryKeyField.GoTypeName>>) error {
	if doc, ok := self.dict[<<.CapCamelSecondaryKey>>]; !ok {
		return gen_server.ErrDataNotFound
	} else {
		// 已经准备删除也，也当成成功返回
		if _, ok := self.del[doc.Id]; ok {
			return nil 
...
func (self *<<.MongoDaoStructName>>) UseClient(client gen_server.DbClient) error {
	switch c := client.(type) {
	case gen_server.MongoClient:
...
func (self *<<.MongoDaoStructName>>) Delete(ctx context.Context, doc *<<.StructName>>) error {
	if doc == nil {
		return gen_server.ErrNullPointer
	}
	if doc.none {
		return gen_server.ErrDataNotExist
	}
	if doc.Id.IsZero() {
		return gen_server.ErrDataNotExist
	}
	...
	if result, err := coll.DeleteOne(ctx, bson.D{{"_id", doc.Id}}); err != nil {
		...
		if result.DeletedCount != 1 {
			return gen_server.ErrDataDeleteFail
		}
	}
	doc.dirty = false
	doc.none = true
...
func (self *<<.RedisDaoStructName>>) UseClient(client gen_server.DbClient) error {
	switch c := client.(type) {
	case gen_server.RedisClient:
...
type gen_state struct {
	config      gen_server.GenModelConfig
...
// 生成协议
func Gen(config gen_server.GenModelConfig) error {
